        ...
        <div className="min-h-screen bg-zinc-900 p-4 flex flex-col items-center justify-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {OptionCards.map((OptionCard, index) => (
                    <GameOptionsCard  Id={OptionCard.id} key={index} sendSelectedOptionCardData={getSelectedOptionCardData} optionImage={OptionCard.optionImg} CategoryText={OptionCard.categoryTxt} />
                ))}
            </div>
        </div>
    );
